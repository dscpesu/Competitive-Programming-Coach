...
int * LeftRotate1(int *arr, int n, int d)    {
    //cpc tag: arith
    d %= n;
    
    //cpc tag: check
    if (d > n/2)    {
        //cpc tag: decl loop ptr swap
        int r = n - d;
        for (int i = 0; i < r; ++i) {
            int temp = arr[n - 1];
            for (int t, j = 0; j < n; ++j)    {
...
int * LeftRotate2(int *arr, int *a, int n, int d)    {
...
int main()
{
    ...
    int *arr = LeftRotate1(a, n, d);
    
    //int *arr = (int *) malloc(n * sizeof(int));
    //arr = LeftRotate2(arr, a, n, d);
